...
本文首先说明 Game Asset Management System（下文简称 GAMS）截至 2026-07-31 的当前正式运行模型。标记为“目标”的章节记录 M7 及以后尚未实现的生产能力，不得把目标状态解读为当前能力。本文把 M5 旧媒体升级作为显式、可审计的一次性迁移操作，不把它混入普通扫描过程。
